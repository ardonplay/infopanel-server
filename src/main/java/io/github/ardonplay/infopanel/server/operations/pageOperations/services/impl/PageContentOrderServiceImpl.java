...
import io.github.ardonplay.infopanel.server.operations.pageOperations.services.PageContentOrderService;
...
@Service
@AllArgsConstructor
public class PageContentOrderServiceImpl implements PageContentOrderService {
...
//                (content.getPageElementType().getName()), content.getBody()), content));
    ...
    @Transactional
    public List<PageContentOrder> updateOrder(PageEntity page, List<Pair<PageElementType, Map<String,JsonNode>>> contentPairs, List<PageContent> pageContents) {
        ...
        int currentIndex = 1;
        for (Pair<PageElementType, Map<String, JsonNode>> currentPair : contentPairs) {
            Pair<Integer, Pair<PageElementType, Map<String, JsonNode>>> pairKey = new Pair<>(currentIndex, currentPair);
